...
export default function Contactpage() {
  ...
  //  console.log(useSelector((state) => state.users.accesstoken))
  useEffect(() => {
    if (showNotification) {
   ...
   const notif = (message)=>{
      return <div id="toast-undo" class="flex items-center w-full max-w-xs p-4 text-gray-500 bg-white rounded-lg shadow dark:text-gray-400 dark:bg-gray-800" role="alert">
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        let  contacts = responses.data.map((obj => {
          const users = Object.entries(obj)
            .filter(([key, value]) => key.startsWith('user') && !key.startsWith('userId'))
            .filter(([key, value]) => value._id !== user._id)
        ...
        contacts = contacts.flatMap(obj => Object.values(obj))
